...
- ✅ `.agents/self/<own>/.openai/agent-card.md` — optional backend-local identity card, only if it already exists.
...
- ❌ Your own `AGENTS.md` / your own `.openai/skills/**` — those are team-tidy's territory (cross-cutting changes shaped
...
- **Capability descriptions still match current skills.** List the directories in your `.openai/skills/`. For each,
