...
The values come from your **Identity** section in AGENTS.md (the agent's system prompt). Same skill works across the
self-agent family because each agent's AGENTS.md owns its own values; this file just describes the procedure.

## Instructions

Read these from AGENTS.md:
...
The two `git config --get` calls should print exactly what your AGENTS.md says. Anything else means a previous setting
...
- Improvising identity values that aren't in AGENTS.md (always ask the user; the system prompt is the source of truth)
