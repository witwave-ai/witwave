...
On the Codex backend, memory file tools are rooted at `/workspaces/witwave-self/memory/agents/mira/`. When using
`read_memory_file`, `write_memory_file`, `append_memory_file`, or `list_memory_files`, pass paths relative to that root
(for example, `platform-health/snapshots/2026-05-22.jsonl`), not absolute `/workspaces/...` paths.
