...
| Focused regression tests              | Partial | Full    | Partial | Full    | Codex now has split auth, health, MCP, hooks, and memory coverage; some tests remain in one large file. |
...
2. Codex test shape: keep splitting the large Node test file into focused contract tests for tracing and metrics.
