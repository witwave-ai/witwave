...
| Focused regression tests              | Partial | Full    | Partial | Full    | Codex now has split auth, health, and MCP coverage; other tests remain in one large JS file.           |
...
2. Codex test shape: keep splitting the large Node test file into focused contract tests for hooks, memory, tracing, and
   metrics.
