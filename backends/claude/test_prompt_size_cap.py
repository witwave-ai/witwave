"""Regression coverage for the claude executor's prompt-size cap (#1620).
...
log/redaction path, and forwarded to the Claude Agent SDK before any
...
We follow the sibling backend tests: pin the source shape with regexes
(so the cap can't silently regress) and unit-test the small isolatable
pieces (the exception type + the metric registration) without importing
the full ``executor`` module — the Claude SDK import chain is too heavy
...
class PromptSizeCapMetricRegistrationTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # `test_health_readiness.py` installs a `_Metric` test-double
    ...
    def test_counter_increments(self):
        labels = {"agent": "test", "agent_id": "test", "backend": "claude"}
