...
Mirrors the cycle-1 claude #1608 fix shape and the codex #1672 follow-
up. README documents ``/health/ready`` as a universal route across all
three backends but gemini shipped neither the readiness flag flip path
nor the route — so K8s readinessProbe could never observe a degraded
gemini pod, and /health conflated liveness with readiness.
...
2. ``/health/ready`` (readiness) returns 503 while ``_ready`` is False
   (still starting OR readiness dropped by ``_guarded()``'s critical-
   task circuit-breaker) and 200 only when fully ready.
...
# ---------------------------------------------------------------------------
# Stub backends/gemini siblings that main.py imports but we don't need to
...
_stub_sibling("executor", {"AgentExecutor": type("AgentExecutor", (), {}), "MAX_SESSIONS": 10000})
_stub_sibling(
    "conversations",
    {
...
class HealthReadinessSplitTests(unittest.TestCase):
    def setUp(self):
        ...
        main._ready = False
        # Leave _health_executor_ref None so the /health subsystem snapshot
        # short-circuits — we're testing routing/status, not subsystem
        # introspection (#1099 has its own coverage).
        main._health_executor_ref = None
    ...
    def test_health_ready_503_when_not_ready(self):
        ...
        or readiness dropped by _guarded()'s critical-task circuit-breaker).
        """
        main._ready = False
        resp = _run(main.health_ready(_make_request()))
